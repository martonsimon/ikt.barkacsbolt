const product_wrapper_id = "product-wrapper";
...
function createProductHTML(item) {
    ...
    <div class="info">
      ...
      <div class="d-flex justify-content-between align-items-center">
        <div>
          <span class="price">${toHUF(item.price)}</span>
          <span class="unit-price">${toHUF(item.unit_price)}/${item.unit}</span>
        </div>
        <div class="order">
          <div class="input-group mb-0">
            <input type="number" class="form-control" placeholder="${item.unit}" id="amount">
            <button class="btn btn-outline-success" type="button" id="button-addon2" onclick="addToCart();">Kosárba</button>
          </div>
        </div>
      </div>
    </div>
...
function loadProduct(){
    ...
    var itemID = queryParams ? queryParams.item || "0" : "0";
    ...
    categoryNavBack.href = `pages/products.html?category=${item.category_id}`;
}
function addToCart(){
    var amountInp = document.getElementById("amount");
    var amount = amountInp.value;
    if (amount){
